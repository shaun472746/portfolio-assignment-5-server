...
const getAllProjectsFromDB = async () => {
    const projects = await ProjectModel.find({deleted:false}).select("-updatedAt -deleted -createdAt");
    return projects;
}
...
const updateProjectIntoDB = async (
    ...
    const result = await ProjectModel.findByIdAndUpdate(id, project, { new: true });
    return result;
  };
